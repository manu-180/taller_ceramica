...
class EliminarUsuario {
  Future<void> eliminarDeBaseDatos(int userId) async {
    ...
    final dataClases = await ObtenerTotalInfo(supabase: supabase, usuariosTable: 'usuarios', clasesTable: taller).obtenerClases();
    final dataUsuarios = await ObtenerTotalInfo(supabase: supabase, usuariosTable: 'usuarios', clasesTable: taller).obtenerUsuarios();
    ...
    for (var clase in dataClases) {
      if (clase.mails.contains(user)) {
        ...
        alumnos.remove(user);
        await supabase.from(taller).update({'mails': alumnos}).eq('id', clase.id);
