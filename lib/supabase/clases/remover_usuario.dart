...
class RemoverUsuario {
  ...
  if (clase.mails.contains(user)) {
    ...
    await supabaseClient.from(taller).update({'mails': clase.mails}).eq('id', idClase);
    ...
    if (clase.espera.isNotEmpty) {
      ...
      print("LA LISTA DE ESPERA ES: $copiaEspera");

      for (final userEspera in copiaEspera) {
        if (await ObtenerClasesDisponibles().clasesDisponibles(userEspera) > 0) {
          clase.mails.add(userEspera);
          clase.espera.remove(userEspera);

          await supabaseClient.from(taller).update({'espera': clase.espera}).eq('id', idClase);
          await supabaseClient.from(taller).update({'mails': clase.mails}).eq('id', idClase);
